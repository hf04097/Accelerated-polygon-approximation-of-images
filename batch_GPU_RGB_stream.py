...
class Individual(object):
    ...
    def __init__(self, mother=None, father=None):
        ...
        if mother is None and father is None:
            ...
            # d_genome = cuda.to_device(self.genome)
            # grid = (8,8)
            # block = (8,8)
            # self.mutate[grid, block](d_genome)
            # self.genome = d_genome.copy_to_host()
...
class Batch():
    ...
    def clear_batch(self):
        ...
        blocks = (math.ceil(WIDTH * HEIGHT / THREADS))  # 320 * 320 / 32
        ...
        batch_difference[blocks, threads_per_block, s1](IMAGE_ARRAY_GPU_0, TARGET_CHANNEL_GPU_0, DIFFERENCES_CHANNEL_0,
                                                        WIDTH * HEIGHT, n)
        batch_difference[blocks, threads_per_block, s2](IMAGE_ARRAY_GPU_1, TARGET_CHANNEL_GPU_1, DIFFERENCES_CHANNEL_1,
                                                        WIDTH * HEIGHT, n)
        batch_difference[blocks, threads_per_block, s3](IMAGE_ARRAY_GPU_2, TARGET_CHANNEL_GPU_2, DIFFERENCES_CHANNEL_2,
                                                        WIDTH * HEIGHT, n)
        # cuda.synchronize()
        # row wise sum of the matrix, such that each row represents the total different
        vec_sum_row[n, threadsperblock_add, s1](DIFFERENCES_CHANNEL_0, SUMS_GPU_0)
        vec_sum_row[n, threadsperblock_add, s2](DIFFERENCES_CHANNEL_1, SUMS_GPU_1)
        vec_sum_row[n, threadsperblock_add, s3](DIFFERENCES_CHANNEL_2, SUMS_GPU_2)
        # row_sum[blocks, threads_per_block](DIFFERENCES_CHANNEL_1, SUMS_GPU)

        # getting summs back to CPU
        SUMS_GPU_0.copy_to_host(sums_1, stream=s1)
        SUMS_GPU_1.copy_to_host(sums_2, stream=s2)
        SUMS_GPU_2.copy_to_host(sums_3, stream=s3)
        ...
        for i in range(n):
            ...
            current_image.fitness = 1 - illness
...
def run_iterations(gens, pop_size, debug = False, validation = False):
    times = []
    sizes = []
    print("starting computing for GPU in batch mode with streams:)")
    if validation:
        global VALIDATION
        VALIDATION = True
